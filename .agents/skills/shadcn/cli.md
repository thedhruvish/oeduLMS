...
> **IMPORTANT:** Always run commands using the project's package runner: `bunx --bun shadcn@latest`. Check `packageManager` from project context to choose the right one. Examples below use `bunx shadcn@latest` but substitute the correct runner for the project.
